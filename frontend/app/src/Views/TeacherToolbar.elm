...
view session page subtools =
    div [ id "teacher-toolbar", class "print:none flex justify-between flex-wrap" ]
        [ ul [ class "flex-1 flex border-b mr-4" ]
...
btn id_ page link txt =
    if page == link then
        li [ class "-mb-px mr-1" ]
            [ a [ id id_, class "bg-white inline-block border-l border-t border-r rounded-t py-2 px-4 text-blue-600  font-semibold", href (routeTo link) ] [ text txt ]
            ]

    else
        li [ class "mr-1" ]
            [ a [ id id_, class "bg-white inline-block py-2 px-4 text-blue-500 hover:text-blue-600 font-semibold", href (routeTo link) ] [ text txt ]
            ]
